...
summary_se_within_cis <- function(data,measurevar, betweenvars=NULL, withinvars=NULL,idvar=NULL, na.rm=FALSE, conf.interval=.95) {
  summarized_data <- summarySEwithin(data=data,measurevar=measurevar,withinvars=withinvars,idvar=idvar,na.rm=na.rm, conf.interval=conf.interval) %>%
    mutate(
      lowerCI = get(measurevar) - ci,
      upperCI = get(measurevar) + ci
    ) %>%
    select(-se, -sd,-ci,-paste0(as.name(measurevar),"_norm"))
} 
```
...
overallNonVLearn <- subjAcc %>%
  ...
  dplyr::summarize(acc=mean(accuracy,na.rm=T),
            sd = sd(accuracy),
            rt=mean(rt))
...
final_accuracy <- all_data %>%
  ...
  group_by(subjCode,trialType) %>%
  dplyr::summarize(N=n(),accuracy=mean(isRight)) %>%
  ungroup()
...
ggsave(here::here("plots","exp_all_frequencyEffect.pdf"), width=18, height=14,dpi=600)
```
...
## Interactions between Trial Number and Frequency {.tabset}
